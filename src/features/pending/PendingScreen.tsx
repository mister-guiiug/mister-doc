...
export function PendingScreen() {
  ...
  async function handleDelete() {
    if (
      !(await confirm({
        message: t('pending.deleteConfirm'),
        danger: true,
    ...
    <div className="grid min-h-dvh place-items-center p-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-lg dark:border-slate-700 dark:bg-slate-800">
        ...
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
          {t('pending.greeting', { name: doctor?.name ?? '' })}
        </p>
        ...
        <div className="mt-6 flex flex-col items-center gap-3">
          ...
          >
            <LogOut className="size-4" /> {t('pending.signOut')}
          </button>
          <button
